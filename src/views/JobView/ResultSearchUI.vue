    ...
    <section class="max-w-7xl mx-auto px-4">
      ...
      <div class="flex max-md:flex-col gap-4 mt-4">
        ...
        <div class="w-72 max-md:w-full" v-show="showFilter">
          <form class="bg-gray-50 px-6 py-4 rounded-md shadow-lg w-full">
        ...
        >
          <template v-if="isLoaded">
            <template v-if="jobList.length > 0">
              <div class="m-6 max-md:m-0 max-md:mt-4" v-for="job in jobList">
                <!-- icon with dropdown -->
                <div class="relative group">
                  ...
                  >
                    <div class="flex flex-col">
                      ...
                      <button
                        class="py-2 px-4 text-left hover:rounded-md hover:shadow-inner hover:bg-indigo-100"
                        v-on:click="copyLinkJob(job.item.id)"
                      >
                        Copy Link
                      </button>
                      <button
                        class="py-2 px-4 text-left hover:rounded-md hover:shadow-inner hover:bg-indigo-100"
                        v-on:click="addToWishlist(job.item.id)"
                        v-if="
                          user.id &&
                          wishlist.filter((wish) => wish.job_id === job.item.id)
                            .length === 0
                        "
                      >
                        Add to Wishlist
                ...
                >
                  ...
                  <div class="">
                    ...
                    <div class="flex gap-6 mb-1 max-md:flex-col max-md:gap-1">
                      <router-link
                        class="hover:underline cursor-pointer"
                        v-bind:to="'/company/' + job.item.company.id"
                        >{{ job.item.company.name }}</router-link
                      >
                      ...
                      <p class="flex items-center">
                        <i class="bi bi-cash"></i>&nbsp;<span
                          >RM{{ job.item.min_monthly_salary }}</span
                        >
                      </p>
                    </div>
                  </div>
                </div>
                <div class="mt-4 border-t-[1.5px] border-indigo-200"></div>
              </div>
            </template>
            <div class="m-6 max-md:m-0 max-md:mt-4" v-else>
              <h4 class="text-lg text-indigo-300 italic">No result...</h4>
            </div>
          </template>
