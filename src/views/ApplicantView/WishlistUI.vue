...
onMounted(async () => {
  ...
  if (response.status !== 200) {
    toggleToastMsg("Failed to get applied job list. Please reload browser.");
  } else {
    wishlist.value = res.data;

    wishlist.value.forEach(async (appl) => {
      const response2 = await fetch(baseEndpoint + "/job/" + appl.job_id, {
        method: "GET",
        mode: "cors",
        headers: { "Content-Type": "application/json" },
        credentials: "same-origin",
  ...
  if (response.status !== 200) {
    ...
    jobList.value.forEach((job) => {
      if (job.recruitment_status === "Advertised") {
        availabilityStatus.value = [...availabilityStatus.value, "applicable"];
      } else {
        availabilityStatus.value = [...availabilityStatus.value, "not"];
      }
    });
                  ...
                  <div class="flex gap-6 mb-1 max-md:flex-col max-md:gap-1">
                    ...
                    <p class="flex items-center">
                      <i class="bi bi-cash"></i>&nbsp;<span
                        >RM{{ job.min_monthly_salary }}</span
                      >
                    </p>
                  </div>
                  <h3>
                    ...
                    <span class="capitalize">{{
                      availabilityStatus[jobList.indexOf(job)]
                    }}</span>
                  </h3>
                </div>
              </div>
      ...
      >
        <div class="border-2 border-indigo-200 rounded-md p-4">
          ...
          <h2 class="text-4xl text-center" v-else>
            {{
              availabilityStatus.filter((avStatus) => avStatus === "applicable")
                .length
            }}
          </h2>
        ...
        <div class="border-2 border-indigo-200 rounded-md p-4">
          ...
          <h2 class="text-4xl text-center" v-else>
            {{
              availabilityStatus.filter((avStatus) => avStatus === "not").length
            }}
          </h2>
