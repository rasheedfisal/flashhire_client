...
async function updateStatusRecruit() {
  ...
  const res = await response.json();
  if (response.status !== 200) {
    toggleToastMsg("Failed to update recruitment status. Please try again.");
  } else {
    toggleFormStatus();
...
async function addNotes() {
  ...
  const res = await response.json();
  if (response.status !== 200) {
    toggleToastMsg("Failed to add notes to this candidate. Please try again.");
  } else {
    toggleModelNotes();
  ...
  <div class="max-w-7xl mx-auto px-6 max-md:px-3">
    ...
    >
      <div class="mt-2 flex items-center gap-2">
        <router-link
          v-bind:to="`/workspace/recruitment/` + job.id + `/successful-list`"
      ...
      <div class="mt-4">
        ...
        <p class="text-center">
          <small
            >Last updated :
            {{ dayjs(job.last_modified_date).format("D MMMM YYYY") }}</small
          >
        </p>
      </div>
      <div class="mt-4">
      ...
      <div class="mt-4 flex gap-4 max-md:flex-col">
        <img
          v-bind:src="success.candidate_profile.user.pic"
          alt=""
        ...
        <div class="max-md:text-[0.9rem]">
          ...
          <div class="mb-4">
            ...
            <ul class="list-disc list-inside px-2">
              <template v-if="success.candidate_profile.education.length > 0">
                <li v-for="edu in success.candidate_profile.education">
                  {{ edu.study_field }} at {{ edu.institute_name }} ({{
                    dayjs(edu.graduation_date, "D MMMM YYYY").year()
                  }})
                </li>
              </template>
              <li v-else>-</li>
          ...
          <div class="mb-4">
            ...
            <ul class="list-disc list-inside px-2">
              <template
                v-if="success.candidate_profile.work_experience.length > 0"
              >
                <li v-for="exp in success.candidate_profile.work_experience">
                  {{ exp.duration }} experience at {{ exp.company_name }} as
                  {{ exp.position }} ({{
                    dayjs(exp.start_date, "D MMMM YYYY").year()
                  }}
                  -
                  {{ dayjs(exp.end_date, "D MMMM YYYY").year() }})
                </li>
              </template>
              <li v-else>-</li>
            </ul>
          ...
          <div class="mb-4">
            ...
            <ul class="list-disc list-inside px-2">
              <template
                v-if="success.candidate_profile.lang_ability.length > 0"
              >
                <li v-for="lang in success.candidate_profile.lang_ability">
                  {{ lang.language_name }}: {{ lang.scale_of_speaking }} out of
                  5 for speaking and {{ lang.scale_of_writing }} out of 5 for
                  writing
                </li>
              </template>
              <li v-else>-</li>
          ...
          <div class="mb-4">
            ...
            <ul class="list-disc list-inside px-2">
              <template v-if="success.candidate_profile.skill.length > 0">
                <li v-for="sk in success.candidate_profile.skill">
          ...
          <div class="mb-4">
            ...
            <ul class="list-disc list-inside px-2">
              <li v-if="success.candidate_profile.resume.length > 0">
                <span
          ...
          <div class="mb-4">
            <h4 class="text-lg">
              <i class="bi bi-blockquote-left"></i>&nbsp;&nbsp; Notes
            </h4>
            <p class="text-gray-400 px-6" v-if="notes === ''">No notes added</p>
            <p class="px-6" v-else>
              {{ notes }}
            </p>
          </div>
          <div class="mt-6 flex gap-2 max-md:flex-col max-md:text-[1rem]">
            <button
                ...
                >
                  ...
                  >
                    <OptionCustom
                      ...
                      "
                      v-bind:class="{
                        'rounded-md shadow-inner bg-indigo-100': status === opt,
                      }"
                    />
                  </template>
                </SelectCustom>
      ...
      >
        <br />
        <br />
        <PdfRenderer v-bind:link="success.candidate_profile.resume[0].path" />
      </DialogModal>
      <DialogModal
