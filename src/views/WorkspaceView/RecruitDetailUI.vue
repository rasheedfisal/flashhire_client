...
async function submitUpdateStatus() {
  ...
  if (response.status !== 200) {
    toggleToastMsg("Failed to update the job recruitment. Please try again.");
  } else {
...
async function deleteRecruitment() {
  ...
  const res = await response.json();
  if (response.status !== 200) {
    toggleToastMsg("Failed to delete the job recruitment. Please try again.");
  } else {
    ...
    >
      <template v-if="isLoaded">
        ...
        <div class="mt-4">
          ...
          <p class="text-center">
            <small
              >Last updated :
              {{
                dayjs(jobRecruit.last_modified_date).format("D MMMM YYYY")
              }}</small
            >
          </p>
        </div>
        ...
        <div class="mt-4">
          ...
          <p>
            RM{{ jobRecruit.min_monthly_salary }}&nbsp;&nbsp;-&nbsp;&nbsp;RM{{
        ...
        <div class="mt-4">
          ...
          >
            <router-link
              ...
              v-bind:to="
                `/workspace/recruitment/` + jobRecruit.id + `/applicant-list`
              "
            >
            ...
            <router-link
              class="border-2 border-indigo-200 rounded-md p-4 hover:border-indigo-400"
              v-bind:to="
                `/workspace/recruitment/` + jobRecruit.id + `/shortlist`
              "
            >
              <h2 class="text-4xl max-md:text-3xl text-center">
                {{ shortlistLength }}
              </h2>
              <p class="mt-2 text-center max-md:text-[0.9rem]">Shortlist</p>
            </router-link>
            <router-link
              class="border-2 border-indigo-200 rounded-md p-4 hover:border-indigo-400"
              v-bind:to="
                `/workspace/recruitment/` + jobRecruit.id + `/interview-list`
              "
            >
              <h2 class="text-4xl max-md:text-3xl text-center">
                {{ interviewLength }}
            ...
            <router-link
              class="border-2 border-indigo-200 rounded-md p-4 hover:border-indigo-400"
              v-bind:to="
                `/workspace/recruitment/` + jobRecruit.id + `/successful-list`
              "
            >
              <h2 class="text-4xl max-md:text-3xl text-center">
                {{ successfulLength }}
        ...
        >
          <br class="mb-2" />
          <form v-on:submit.prevent="deleteRecruitment()" class="max-w-lg">
            <p>
              Are you sure want to delete this recruitment
              <strong>permanently</strong>? Any data related to it will be
              <strong>deleted too</strong>!
            </p>
            <div class="mt-10">
              <label class="mb-2 block"
                >Please type your company name and this job designation</label
              >
              <input
                type="text"
